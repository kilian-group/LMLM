...
def remove_dblookup_patterns(text, triplets_to_keep=None, triplets_to_remove=None):
    """
    ...
    Args:
        ...
        triplets_to_keep (list): A list of tuples (entity, relationship, return_value) to keep.

    Returns:
        str: The text with unwanted [dblookup( )] patterns removed.
    """
    import re
...
if __name__ == '__main__':
    ...
    save_path = args.input_path.replace(".jsonl", f"_factscore_output.json")
    if os.path.exists(save_path) and os.path.getsize(save_path) > 0:
        print(f"Already exists at", save_path)
        exit()
    ...
    ## move all dblookup for LMLM
    cleaned_generations = []
    for generation in generations:
        cleaned_generations.append(remove_dblookup_patterns(generation, triplets_to_keep=[]))
    ...
    # "score": NaN,
    if isinstance(out['score'], float) and math.isnan(out['score']):
        raise ValueError(f"score is NaN. Please check the input data.")
